...
public class UpdateDatepickerAnni : MonoBehaviour
{
    ...
    void SetDefaultToToday()
    {
        ...
        if (getid == 1) 
        { 
            today = ConvertStringToDate(PlayerPrefs.GetString("MaleAniDate")); 
        }
        else if (getid == 2) 
        {
            today = ConvertStringToDate(PlayerPrefs.GetString("FeMaleAniDate")); 
        }
    ...
    void UpdateDateText()
    {
        ...
        updateprofileins.anni_Text = selectedDateText.text; //put condition
        updateprofileins.aniDate = new System.DateTime(year, month, day);
    }
