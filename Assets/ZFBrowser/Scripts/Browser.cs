...
")]
	...
	public INewWindowHandler NewWindowHandler;
	...
	protected DialogHandler dialogHandler;
		...
		onNativeReady += id => {
			...
			Debug.LogError(Url+"Url is call ");
		};
		...
		onConsoleMessage += (message, source) => {
			...
			if (text.Contains("Thank you,your payment was successful,now close this tab"))
			{
				Debug.LogError("clode tab is call ....... ");
				//Success Payment
				VerifyPayment();
                Destroy(this.gameObject);
			}
			else if(text.Contains("Thank you,your payment was canceled,now close this tab"))
			{
				//Failed Payment
                Destroy(this.gameObject);
            }
